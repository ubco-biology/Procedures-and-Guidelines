...
* Parameter estimates (i.e. mean) should be accompanied by measures of uncertainty, i.e. the *standard error of the mean* (SEM); confidence *interval* (notation: lower limit – upper limit); or confidence *limits*: (lower limit, upper limit).
...
* Your *Methods* section should clearly state your significance level (α), which should be decided prior to the beginning of the study.
* Test statistics (e.g. Student’s *t* or an \"F\" from ANOVA) should be rounded to 2 decimal places, and associated *P*-values should report 3 decimal places. If your *P*-value is less than 0.001, it should be reported as <0.001. *P*-values do not indicate effect size, so reporting *P* = 10^-6 is not more impressive than *P* <0.001.
