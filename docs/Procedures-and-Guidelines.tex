...
\item
  Parameter estimates (i.e.~mean) should be accompanied by measures of uncertainty, i.e.~the \emph{standard error of the mean} (SEM); confidence \emph{interval} (notation: lower limit -- upper limit); or confidence \emph{limits}: (lower limit, upper limit).
\item
...
\tightlist
\item
  Report a frequency table for the raw data
\item
  Alternatively, since proportions are the main descriptive statistic of interest, provide a summary table with proportions for each category. Include the confidence interval for the proportion if appropriate.
\end{itemize}
...
\tightlist
\item
  Your \emph{Methods} section should clearly state your significance level (α), which should be decided prior to the beginning of the study.
\item
  Test statistics (e.g.~Student's \emph{t} or an "F" from ANOVA) should be rounded to 2 decimal places, and associated \emph{P}-values should report 3 decimal places. If your \emph{P}-value is less than 0.001, it should be reported as \textless0.001. \emph{P}-values do not indicate effect size, so reporting \emph{P} = 10\^{}-6 is not more impressive than \emph{P} \textless0.001.
\item
  In the absence of a table, your concluding statement should include the name of the test, test statistic value, degrees of freedom (df) or sample sizes, P-value, and confidence interval (if appropriate). All of this should be in parentheses.
